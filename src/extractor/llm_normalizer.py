...
Правила:
- Включай ТОЛЬКО гранты с активным open call — то есть приём заявок сейчас открыт или откроется в будущем.
- КРИТИЧНО: если дедлайн уже прошёл относительно CURRENT_DATE — НЕ включай вообще, пропускай полностью.
- Если страница рассказывает о ПРОШЕДШИХ грантах, лауреатах, победителях — игнорируй полностью, верни [].
...
- amount пиши как строку: "$5,000", "up to €10,000", "varies", "undisclosed". Если сумма неизвестна — пиши "undisclosed", НЕ пиши "FREE" или "free" — это не сумма гранта
- Если один грант имеет несколько региональных циклов (Boston, Atlanta, Houston и т.д.) — верни ОДИН объект с общим названием и самым ранним дедлайном, не дублируй
...
- opportunity_quality: "high" / "medium" / "low" / "reject"
  ...
  low = узкая география / unclear funding
  reject = платная ловушка / строго фотография / строго дизайн / строго кино / строго музыка / не для visual artists / nomination-only (нельзя подать самому)
